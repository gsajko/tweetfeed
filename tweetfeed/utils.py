...
class BatchProcessor:
        ...
        if "data_path" in self.kwargs:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "duplicate values in predictions.csv, resetting scores"
                    )
                    self.df["preds"] = 0
            except FileNotFoundError:
    ...
    def filter_news(self):
        ...
        if self.remove_news:
            self.df = self.df[self.df["contains_news"] == 0].reset_index(
                drop=True
            )[:batch_size]
        else:
            self.df = self.df.reset_index(drop=True)[:batch_size]

    def remove_from_mutelist(self):
        if "mute_list" in self.kwargs:
            self.df = drop_contains(
                self.df, "full_text", self.kwargs["mute_list"]
            )
        if "mute_list_cs" in self.kwargs:
            self.df = drop_contains(
                self.df,
                "full_text",
                self.kwargs["mute_list_cs"],
                case_sensitive=True,
            )
