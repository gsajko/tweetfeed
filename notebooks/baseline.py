...
y_pred = np.random.choice(
    np.arange(0, 2), size=(len(y_test), 1),
    p=[1-sent_perc, sent_perc])
...
performance['log_reg_balanced'] = fit_and_evaluate(LogisticRegression(class_weight="balanced"))
performance['log_reg_p'] = fit_and_evaluate(LogisticRegression(class_weight=p))
weights = {0: 0.03, 1: 0.97}
performance['log_reg_fine_tuned_weights'] = fit_and_evaluate(LogisticRegression(class_weight=weights))
...
for s in solver:
    performance = fit_and_evaluate(LogisticRegression(class_weight="balanced", solver=s))
    print(s)
    print(json.dumps(performance["overall"], indent=2))
