...
# %%
df_seen = df_tweets[df_tweets["id"].isin(reviewed_tweets["tweet_id"].tolist())]
# %%
...
# %%
# prep_batch
df = concat_tweet_text(df_tweets)
df = df[df["retweeted_status"] == "N/A"]
with open("../tweetfeed/data/news_domains.txt", "r") as f:
    news_domains = json.loads(f.read())
df = find_news(df, news_domains)
df_news = df[df["contains_news"] == 1]
# %%
